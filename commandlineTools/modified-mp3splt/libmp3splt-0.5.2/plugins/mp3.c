    ...
    const char *id3, double fbegin_sec, double fend_sec, int *error)
{
  ...
  //zenpho 
  //zenpho fprintf(stdout,"%s,%.2f,%.2f\n",output_fname,fbegin_sec,fend_sec);
  //printf("%s,%.2f,%.2f\n",output_fname,fbegin_sec,fend_sec);
  
  printf("<Turn startTime=\"%.2f\"",fbegin_sec);
  printf(" endTime=\"%.2f\"",fend_sec);
  printf(" splitFilename=\"%s\"", output_fname);
  printf(" speaker=\"spk1\">\n");
  printf("<Sync time=\"%.2f\"/>\n", fbegin_sec);
  printf("***Include transcript words here***\n");
  printf("</Turn>\n");
